...
<AccordionGroup>
  ...
  <Accordion title="Can I import and export tests?" icon="file-import">
    **Import:** OneTest supports importing tests from:
    ...
    - **JUnit XML files** — automated results via Test Agents
  ...
  <Accordion title="How do I integrate with CI/CD?" icon="code-branch">
    OneTest provides **Test Agents** — ReportPortal-compatible API keys for CI/CD integration:

    1. Go to **Settings** > **Test Agents**
    2. Click **+ Add API Key**
    ...
    See [Test Agents](/settings/test-agents) for detailed setup.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="What are MCP servers?" icon="puzzle-piece">
    **MCP (Model Context Protocol)** servers extend the AI Assistant's capabilities by connecting external tools.

    For example, you can connect to Jira, GitHub, Slack, or custom internal tools so the AI can interact with them during conversations.

    1. Go to **Settings** > **MCP Servers**
    2. Browse the registry or add a custom server
    3. Enable the server

    See [MCP Servers](/settings/mcp-servers) for details.
  </Accordion>
